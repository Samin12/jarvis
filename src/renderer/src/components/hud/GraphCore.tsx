...
// ---------------------------------------------------------------------------
// GRAPH CORE — ported from jarvis-hud-1.0.1 (JARVIS reference replica).
...
// playing; falls back to the synthetic envelope.
// Writes --accent-h to document.documentElement every frame — ALL chrome
// accents in globals.css derive from it.
// ---------------------------------------------------------------------------
...
export default function GraphCore({
  ...
  useEffect(() => {
    ...
    cloud.position.y = 0.32 // sit slightly above screen center, clear of the bottom zones
    scene.add(cloud)
    ...
    const tick = (): void => {
      ...
      nodeMat.uniforms.uInner.value.lerp(tInner, 0.06)
      nodeMat.uniforms.uOuter.value.lerp(tOuter, 0.06)
      ...
      if (nebula.visible) {
        ...
        nebMat.uniforms.uCol.value.lerp(tOuter, 0.06)
      }

      halo.material.color.copy(nodeMat.uniforms.uOuter.value)
